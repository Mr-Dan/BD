...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `cost_procedure`(IN TypeD VARCHAR(100),IN persent INT)
BEGIN
DECLARE countId INT DEFAULT 5; 
if(persent!=-100) then
set countId = (select count(IdDish) from dish where TypeDish ='Салат');
